...
for iEvt in range(10000):
  ...
  #The array that holds the hits need to be reset for each event, but this causes an issue. Namely that when the branches are initialized above
  #  they point to a specific memory address, but resetting these arrays changes their memory addresses. However the branch is not aware of this
  #  change, so each branch.Fill() will just fill with junk data each loop.
